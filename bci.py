...
"""
Basic Brain-Computer Interface
=============================================

Description:
This code utilizes an automatic algorithm to recognize somebody's 
mental state from their EEG. It uses a classifier, an algorithm 
that, provided some data learns to recognize patterns,
...
if __name__ == "__main__":
    ...
    feature_names = BCIt.get_feature_names(ch_names)
    ...
    BCIt.beep()  # Beep sound
    eeg_data1, timestamps1 = inlet.pull_chunk(
            timeout=training_length+1, max_samples=fs * training_length)
    eeg_data1 = np.array(eeg_data1)[:, index_channel]

    # Divide data into epochs
    eeg_epochs0 = BCIt.epoch(eeg_data0, epoch_length * fs,
                             overlap_length * fs)
    eeg_epochs1 = BCIt.epoch(eeg_data1, epoch_length * fs,
                             overlap_length * fs)

    """ 4. COMPUTE FEATURES AND TRAIN CLASSIFIER """

    feat_matrix0 = BCIt.compute_feature_matrix(eeg_epochs0, fs)
    feat_matrix1 = BCIt.compute_feature_matrix(eeg_epochs1, fs)

    [classifier, mu_ft, std_ft, score] = BCIt.train_classifier(
    ...
    plotter_decision = BCIt.DataPlotter(30, ['Decision'])
    ...
    try:
        while True:
            ...
            # Update EEG buffer
            eeg_buffer, filter_state = BCIt.update_buffer(
                    eeg_buffer, ch_data, notch=True,
                    filter_state=filter_state)

            """ 3.2 COMPUTE FEATURES AND CLASSIFY """
            # Get newest samples from the buffer
            data_epoch = BCIt.get_last_data(eeg_buffer,
                                            epoch_length * fs)

            # Compute features
            feat_vector = BCIt.compute_feature_vector(data_epoch, fs)
            y_hat = BCIt.test_classifier(classifier,
                                         feat_vector.reshape(1, -1), mu_ft,
                                         std_ft)
            print(y_hat)

            decision_buffer, _ = BCIt.update_buffer(decision_buffer,
